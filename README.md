...
```powershell
ssh -L 6080:localhost:6080 -L 8000:localhost:8000 root@你的ECS公网IP
```
...
```bash
curl -H "Authorization: Bearer change-me" http://localhost:8000/browser-status
```
...
```bash
curl -X POST -H "Authorization: Bearer change-me" http://localhost:8000/browser/attach
curl -H "Authorization: Bearer change-me" http://localhost:8000/browser-status
```
