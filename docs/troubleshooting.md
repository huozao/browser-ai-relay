...
GitHub Actions 自动部署时，ECS 上 `/root/browser-ai-relay` 应作为部署 checkout 使用，不建议手动修改 tracked 文件。新版 workflow 在备份私有 env 后会执行 `git reset --hard origin/main`，直接把 tracked 文件同步到远端版本，避免 `git pull --ff-only` 被 `deploy/ecs/*.sh` 等本地改动阻塞。私有配置仍只应放在 ignored 文件里，例如 `deploy/ecs/release-meta.env`。
